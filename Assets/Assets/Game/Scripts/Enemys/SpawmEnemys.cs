...
public class SpawmEnemys : MonoBehaviour
{
    ...
    IEnumerator Spamear()
    {
        GameObject tmp =  Instantiate(PrefabEnemy, transform.position, Quaternion.identity); // 5
        tmp.GetComponent<EnemyClass>().SetLevelEnemy(levelEnemigo, objetivo);//2
        _enemyCreado?.Invoke(tmp.GetComponent<EnemyClass>());//1
        ListaEnemigos.AddOndeStart(tmp.GetComponent<EnemyClass>());//1
        yield return new WaitForSecondsRealtime(intervaloSpawm);
        Debug.Log(ListaEnemigos.count);//2
        if (objetivo.transform.position.y > 15)//1
        {
            MiPiso = false;//1
        }
        if (MiPiso==true)
        {
            StartCoroutine(Spamear());//1
        }        
    }
}
